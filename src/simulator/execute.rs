...
pub fn excute_warp<F>(bit_count: usize, objective_function: F, config: AnnealingConfig) -> Vec<f64>
where
    F: Fn(usize) -> f64,
{
    ...
    let develop_time_warp = module.load_function("develop_time_warp").unwrap();
    let calc_norm = module.load_function("add_to_calc_norm").unwrap();
    let update_f0 = module.load_function("update_f0").unwrap();
    ...
    let cfg_for_vector = create_launch_config(n, config.threads_x, KernelLayout::Vector2D);
    let cfg_for_warp = create_launch_config(n, config.threads_x, KernelLayout::Warp);
    ...
    for i in 0..step {
        ...
        unsafe  {
            ...
            match stream
                .launch_builder(&develop_time_warp)
                .arg(&a)
                .arg(&b)
                .arg(&config.dt)
                .arg(&diag_dev)
